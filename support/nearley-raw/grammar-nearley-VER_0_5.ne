...
#---------------------------------------------------------------
logical_expr ->
#"not" _S logical_operand {% d => ({not:d[2]}) %}

 		logical_operand _ ("or" | "and") _ logical_operand
		{% d => ({
			logic:{
					left:d[0],
					operator:d[2][0],
 		...
 		|logical_expr _ ("or" | "and") _ logical_operand
		{% d => ({
			logic:{
					left:d[0],
					operator:d[2][0],
...
logical_not_operand -> "not" _ OPERAND {% d => ({not:d[2]}) %}
...
bitarray_inner -> (bitarray_expr | _S "," _ bitarray_expr):+
{% d => {
	var arr = [].concat(...d);
	return arr.join('');
} %}
...
point4 -> ("[" _) EXPR (_ "," _) EXPR (_ "," _) EXPR (_ "," _) EXPR (_ "]")
		{% d => ({type: 'Point4', 'x':d[1], 'y':d[3], 'z':d[5], 'w':d[7]}) %}
#---------------------------------------------------------------
