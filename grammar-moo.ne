...
# ROLLOUT DEFINITION --- OK
    ...
    #---------------------------------------------------------------
    # item_type -> "label" | "button"
    item_type -> %kw_uicontrols
#---------------------------------------------------------------
# MACROSCRIPT --- OK ?
    macroscript_def ->
    ("macroscript" __) var_name macro_script_param:* _
    LPAREN
...
# CONTEXT EXPRESSION -- UNFINISHED
    ...
    | ("with" __):? %kw_context _ logical_expr
...
# MATH EXPRESION ---  needs to drop operand results..
    ...
    #math_expr ->
    #math_operand _ mathSym _ math_expr
...
# LOGIC EXPRESION --- this probably wont work
    ...
    logical_operand __ %kw_compare __  (not_operand | logical_operand)
    {% d => ({
        'operand_A':d[0],
        'operator': d[2],
        'operand_B':d[4]
    ...
    | logical_expr __ %kw_compare __  (not_operand | logical_operand)
    {% d => ({
        'operand_A1':d[0],
        'operator': d[2],
        'operand_B1':d[4]
    ...
    not_operand -> %kw_not __ logical_operand {% d => ({'not':d[2]}) %}
...
# PARAMETER CALL --- OK
    ...
    # param_arg_loc -> var_name (_ ":" _) resource
    # {% d =>({
    # 	'name':d[0],
    # 	'operand':d[2]
    # })%}
...
#PARENS - WHITESPACE - EOL
    ...
    #---------------------------------------------------------------
    # EOL -> _S (newline|";"):+ _S
    EOL -> _S ( %newline | %statement ):+ _S  {% d => null %}
#---------------------------------------------------------------
